...
def export_traj_race_f110(file_paths: dict,
    ...
    - kappa_radpm [rad/m]   Curvature (used for feedforward steering in MPC)
    ...
    traj_race:   numpy array of shape [N x 7] with all trajectory columns
    """

    header = "s_m,x_m,y_m,psi_rad,kappa_radpm,vx_mps,ax_mps2"
    fmt = "%.7f,%.7f,%.7f,%.7f,%.7f,%.7f,%.7f"
    with open(file_paths["traj_race_export"], 'w') as fh:
        fh.write("# MPC Raceline Trajectory — generated by Raceline-Optimization\n")
        fh.write("# Columns: s_m [m], x_m [m], y_m [m], psi_rad [rad], kappa_radpm [rad/m], vx_mps [m/s], ax_mps2 [m/s^2]\n")
